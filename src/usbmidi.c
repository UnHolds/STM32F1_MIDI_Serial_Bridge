/* A Converter for serieal MIDI <-> USB MIDI
 *
 ...
 * Normaly USB-MIDI should have a BULK endpoint, but due to the fact that 
 * I want it to be interrupt driven I used an INTERRUPT endpoint 
 * (PC -> converter).
...
/* UART FIFO -> UART writes in this FIFO */
static FIFO uart_FIFO;

/* USB FIFO -> USB writes in this FIFO */
...
int main(void)
{
	 ...
	 * incoming uart data would kill the registration process if we 
         * would not wait
         */
